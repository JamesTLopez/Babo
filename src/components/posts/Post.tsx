...
const Post: React.FC<postValues> = ({ title, hours, date, description }) => {
  let [activate, setActivate] = useState<boolean>(false);
  let [postValues, setPost] = useState<postValues>({title:title,hours:hours,date:date,description:description});
  ...
  return (
    <div className={`post fadeIn h-full sm:w-3/4 xl:w-4/6  bg-gray-800 my-5 rounded-lg flex justify-between flex-col sm:flex-row shadow-xl `}>
      ...
      <div className="flex items-center justify-between flex-col w-full p-2 bg-gray-800 rounded-lg">
        <div className="flex w-11/12 ">
          <div className="flex-1 p-2  text-xs text-white flex flex-col ">
            <h2 className="text-lg font-bold opacity-100">{postValues.title}</h2>
